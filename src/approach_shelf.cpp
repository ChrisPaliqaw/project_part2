...
#include "../include/project_part2/approach_shelf.hpp"
...
{
ApproachShelf::ApproachShelf(
        std::string scan_topic,
        std::string odom_topic,
        std::string cmd_vel_topic):


    Node("approach_shelf_service_server"),
    scan_topic(scan_topic),
    cmd_vel_topic(cmd_vel_topic),
    state_(ApproachShelfState::forward_01)
{
    ...
    timer_ptr_ = this->create_wall_timer(
        0.5s,
        std::bind(&ApproachShelf::timer_callback, this),
        callback_group_);
    ...
    laser_subscription_ = create_subscription<sensor_msgs::msg::LaserScan>(
        scan_topic,
        10,
        std::bind(&ApproachShelf::scan_callback, this, _1),
        laser_options);
    odom_subscription_ = create_subscription<nav_msgs::msg::Odometry>(
        odom_topic,
        10,
        std::bind(&ApproachShelf::odom_callback, this, _1),
        odom_options);

    cmd_vel_publisher_ = create_publisher<geometry_msgs::msg::Twist>(cmd_vel_topic, 10);

    RCLCPP_INFO_STREAM(this->get_logger(), "Create ApproachShelf");
...
geometry_msgs::msg::Vector3 ApproachShelf::euler_from_quaternion(tf2::Quaternion q)
{
...
double ApproachShelf::magnitude(geometry_msgs::msg::Vector3 v3)
{
    return sqrt(v3.x*v3.x + v3.y*v3.y + v3.z*v3.z);
}

bool ApproachShelf::is_buffer_stop_state () const
{
    switch (state_)
    {
    case ApproachShelfState::stop_forward_01:
        [[fallthrough]];
    case ApproachShelfState::stop_turn:
        return true;
    default:
        return false;
    }
...
bool ApproachShelf::is_odom_state () const
{
    switch (state_)
    {
    case ApproachShelfState::stop_forward_01:
        [[fallthrough]];
    case ApproachShelfState::set_turn:
        [[fallthrough]];
    case ApproachShelfState::turn:
        [[fallthrough]];
    case ApproachShelfState::stop_turn:
        return true;
    default:
...
bool ApproachShelf::is_scan_state () const
{
    switch (state_)
    {
    case ApproachShelfState::stop_forward_01:
...
bool ApproachShelf::is_stopped(geometry_msgs::msg::Vector3 v3)
{
    return abs(magnitude(v3)) <= kTurnFuzz;
}

void ApproachShelf::timer_callback()
{
    if (is_complete_)
    {
        return;
    ...
    switch (state_)
    {
    case ApproachShelfState::forward_01:
        forward();
        break;
    case ApproachShelfState::stop_forward_01:
        stop();
...
void ApproachShelf::odom_callback(const nav_msgs::msg::Odometry::SharedPtr message)
{
    if (is_complete_)
    {
        return;
    ...
    else if (is_buffer_stop_state())
    {
        ...
        if (stopped)
        {
            switch (state_) {
            case ApproachShelfState::stop_forward_01:
                state_ = ApproachShelfState::set_turn;
                log_state();
                break;
            case ApproachShelfState::stop_turn:
                state_ = ApproachShelfState::stop;
                log_state();
                break;
            default:
    ...
    else if (state_ == ApproachShelfState::set_turn) {
        RCLCPP_INFO_STREAM(get_logger(), "SETTING TURN");
        tf2::Quaternion q_rot;
        // Rotate the current pose by 90 degrees about Z to get our goal pose
        q_rot.setRPY(0.0, 0.0, ApproachShelf::kGoalAngularDisplacement);
        tf2::Quaternion goal_turn = q_rot * tf2_quaternion;
        goal_turn.normalize();
        goal_turn_v3_ = ApproachShelf::euler_from_quaternion(goal_turn);
        state_ = ApproachShelfState::turn;
    }
    else if ((state_ == ApproachShelfState::turn) && (abs(current_rotation_v3.z - goal_turn_v3_.z) <= kTurnFuzz))
    {
        state_ = ApproachShelfState::stop_turn;
        log_state();
    }
}

void ApproachShelf::scan_callback(const sensor_msgs::msg::LaserScan::SharedPtr message)
{
    // std::lock_guard<std::mutex> lock(state_mutex_);
    if (is_complete_)
    {
    ...
    switch (state_)
    {
        case ApproachShelfState::forward_01:
            ...
            {
                state_ = ApproachShelfState::stop_forward_01;
                log_state();
            }
            break;
        default:
...
void ApproachShelf::turn()
{
    stop();
    twist_->angular.z = this->kRightAngularVelocity;
}

void ApproachShelf::forward()
{
    stop();
    twist_->linear.x = linear_velocity_;
}

void ApproachShelf::publish_twist()
{
    log_velocity();
    cmd_vel_publisher_->publish(*twist_);
}

void ApproachShelf::log_state_verbose() const
{
...
void ApproachShelf::log_state() const
{
    RCLCPP_INFO(get_logger(), state_string(state_));
}

void ApproachShelf::log_velocity() const
{
...
std::string ApproachShelf::state_string(ApproachShelfState state)
{
...
int main(int argc, char *argv[]) {
  ...
  std::shared_ptr<project_part2::ApproachShelf> pre_approach =
      std::make_shared<project_part2::ApproachShelf>();
